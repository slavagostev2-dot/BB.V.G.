...
2. Исполняемый cross-module `wheel_scenario_suite`, который проверяет обнаружение, дубли, публикации, участие, lifecycle и повторное использование ссылки.
3. Наличие обязательных предметных self-test в preflight автоучастия и фактический порядок шагов `auto-participation.yml`, включая быстрый проход, оба дополнительных аккаунта, полный recovery и финальный merge.

`wheel_scenario_suite` запускается в отдельном процессе. Специализированные browser/recovery/outcome self-test остаются обязательной частью production preflight и не запускаются повторно внутри общего `pytest`. Проверки не выполняют реальные сетевые запросы, не открывают браузер, не пишут production JSON и не изменяют пользовательские данные.
