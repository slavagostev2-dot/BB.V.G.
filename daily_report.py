...
def combined_stats(fast_stats: dict, discovery: dict) -> dict:
    ...
    for day, entry in discovery.get("stats_daily", {}).items():
        if not isinstance(entry, dict):
            continue
        day_target = result["daily"].setdefault(
            day, {"sources": {}, "totals": {}}
        )
        merge_numeric_dict(
            day_target.setdefault("totals", {}), entry.get("totals", {})
        )
        for source, source_entry in entry.get("sources", {}).items():
            if not isinstance(source_entry, dict):
                continue
            source_target = day_target.setdefault("sources", {}).setdefault(
                source, {}
            )
...
def truthy_env(name: str) -> bool:
    return str(os.getenv(name, "")).strip().casefold() in {
        "1",
        "true",
        "yes",
        "on",
...
def report_dates(
    period: str, *, current: date, manual: bool
...
def aggregate_period(stats: dict, allowed_dates: list[str]) -> dict[str, Any]:
    ...
    for day, entry in stats.get("daily", {}).items():
        ...
        day_totals = (
            entry.get("totals")
            if isinstance(entry.get("totals"), dict)
            else {}
        )
        ...
        for source, source_entry in source_rows.items():
            ...
            if count > 0:
                source_counts[str(source)] = (
                    source_counts.get(str(source), 0) + count
                )
    ...
    best_day = max(
        day_counts.items(), key=lambda item: item[1], default=("", 0)
    )
    return {
        "totals": totals,
...
def report_text(
    *,
    ...
    if wheel_posts > 0:
        ...
        lines.append(
            f"📡 Источников с находками: <b>{len(summary['source_counts'])}</b>"
        )
        if notifications > 0:
        ...
        if days > 1:
            ...
            if best_day and best_count > 0:
                lines.append(
                    ...
                    f"<b>{datetime.fromisoformat(best_day):%d.%m.%Y}</b> — "
                    f"{best_count}"
                )
        if top_sources:
        ...
        lines.append(
            "За выбранный период новые публикации с колёсами не обнаружены."
        )
    ...
    if active_rows:
        lines.append(
            f"⏱ Время определено: <b>{with_time} из {len(active_rows)}</b>"
        )
        lines.append(
            f"✅ Участие отмечено: "
            f"<b>{len(active_keys & participating)} из {len(active_rows)}</b>"
        )
    ...
    quarantined = sum(
        1 for entry in health_rows if entry.get("status") == "quarantined"
    )
    problems = sum(
        1
        for entry in health_rows
...
def _notification_kind_for_period(period: str) -> str:
    if period == "daily":
        return notification_router.KIND_DAILY_REPORTS
    if period == "weekly":
        return notification_router.KIND_WEEKLY_REPORTS
    # Monthly summaries are manual-only today and historically routed to the
    # administrator. Keep that behavior explicit instead of inventing a new
    # user preference during this migration.
    return notification_router.KIND_ADMIN_SYSTEM
...
def main() -> int:
    ...
    notification_router.send_notification(
        monitor,
        text,
        kind=_notification_kind_for_period(period),
    )
    print(
        f"{BRAND_NAME} {period} summary sent for "
...
def self_test() -> None:
    ...
    assert _notification_kind_for_period("daily") == "daily_reports"
    assert _notification_kind_for_period("weekly") == "weekly_reports"
    assert _notification_kind_for_period("monthly") == "admin_system"
