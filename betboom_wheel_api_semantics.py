...
def classify_info(monitor_module: Any, info: dict[str, Any], *, current=None):
    ...
    # Only an explicit end flag or an actually elapsed server deadline is
    # terminal. ``is_early`` is an early/not-yet-open phase, not evidence that
    # a new wheel has already finished.
...
def self_test() -> None:
    ...
    # Regression for the current zonertg13 shape: the server start is current,
    # duration may be temporarily absent, and is_early must not archive it.
