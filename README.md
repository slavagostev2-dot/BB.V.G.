...
- Реферальные и предположительно реферальные колёса сохраняются, получают одну
  первичную Telegram-карточку и участвуют в автоучастии всех трёх аккаунтов.
  Control Center отправляет один общий итог с честным состоянием каждого
  аккаунта. В VK такие события не отправляются.
