...
def check_discovery_runtime_with_sync_grace(
    ...
    """Do not report a stale discovery snapshot as a current inventory failure.

    Source inventory can change at any time, while candidate discovery is periodic.
    A size mismatch is actionable only shortly after discovery itself has run; an
    older snapshot simply waits for the next scheduled discovery refresh.
    """
    ...
    mismatch = [item for item in added if item.get("kind") == "discovery_inventory"]
    if not mismatch:
        return
    ...
    discovery["inventory_snapshot_age_hours"] = max(
        0, int(age.total_seconds() // 3600)
    )
...
def self_test() -> None:
    ...
    try:
        ...
        def stale_mismatch(details: dict[str, Any], findings: list[dict[str, Any]]) -> None:
            details["discovery"] = {
                "discovery_last_run_at": "2026-07-19T05:00:00+00:00"
            }
        ...
        def fresh_mismatch(details: dict[str, Any], findings: list[dict[str, Any]]) -> None:
            details["discovery"] = {
                "discovery_last_run_at": "2026-07-19T14:00:00+00:00"
            }
            findings.append(legacy.finding(
                "discovery_inventory",
                "Ночная проверка видит не весь утверждённый пул",
