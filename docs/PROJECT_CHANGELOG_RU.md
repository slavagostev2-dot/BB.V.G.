# 2026-08-09 — устранена повторная финализация результата автоучастия

- Принудительное чтение encrypted state больше не заменяет локальный bundle,
  пока его изменение ожидает подтверждённой фоновой записи. Удалённое состояние
  читается самим writer и объединяется существующим трёхсторонним merge.
- Очередь приватного состояния получила монотонное поколение. Более новый save
  с тем же текстом коммита нельзя ошибочно снять завершением предыдущего flush.
- Безопасный порядок блокировок `access → bundle` перенесён к предметному
  владельцу `bbvg/bot/storage.py`; дублирующий `load_access` удалён из
  production compatibility-entrypoint без изменения MRO и callback-контрактов.
- Regression-тесты воспроизводят потерю owner outcome после `force=True`,
  совпадающие commit messages двух последовательных save и порядок блокировок.

**Backup перед изменением:**
`backup/2026-08-09-before-private-state-save-race-repair` →
`db37f5313d0fdf03ab0819d8dc8c83e8089dca15`.
...
- Инциденты `kekw/1183` и `helin/1187` подтвердили повтор первичной
  Telegram-карточки: Monitor доставил текущую генерацию, а более поздний
  recovery-handoff повторил её с другим publication-scoped button token.
- Внутренняя идентичность доставки теперь использует канонический event ID
  `wheel_key + action_id + server_start_at`. Служебное поле удаляется из markup
  до вызова Telegram API; пользовательские callback-строки не изменены.
- Recovery сравнивает существующую активную запись с найденным событием по
  канонической генерации. Запись предыдущего `action_id` больше не может
  получить новое серверное время, сохранив старые источник, пост и button token.
- Regression-сценарии покрывают два разных поста одной генерации, следующую
  генерацию той же ссылки и замену устаревших publication-метаданных.

**Проверенная pre-change точка восстановления:**
`safety/backup-2026-08-02-cross-source-duplicate-prechange-*` — отдельные refs
для production-кода, `main`, `runtime-state`, `runtime-status` и
`runtime-ledger`; контрольные blob SHA совпали с исходными ветками.

# 2026-08-02 — распознавание уже принятого участия и память реферального идентификатора

- Снимок `deko2/1168` доказал, что BetBoom показывал цельный баннер
  «Отлично! Теперь ты участвуешь в розыгрыше», но exact-label detector применял
  `fullmatch` ко всему баннеру и ошибочно сохранял `button_not_found`.
  Детектор теперь принимает однозначную фразу успеха внутри ограниченного
  видимого блока, сохраняя строгий exact-match для кнопок и отрицательный тест
  против текста правил.
- Классификация учитывает явный текст публикации, явный/слабый referral-сигнал
  BetBoom-страницы и сохранённую историю того же идентификатора. Прежняя
  доказанная referral-генерация делает новую неоднозначную генерацию только
  `suspected_referral`; она не создаёт `referral_ineligible` для аккаунта.
- История хранится в существующем `state.json`, ограничена 500 идентификаторами
  и может восстановиться из сохранённых `event_context` прошлых попыток. Новый
  `action_id` не наследует точный referral-флаг старой генерации.
- Первичная Telegram-карточка и список активных колёс показывают
  «Предположительно реферальное колесо», когда вывод основан только на истории
  или слабом баннере. Результаты трёх аккаунтов остаются независимыми и
  монотонными.

**Проверенная pre-change точка восстановления:**
`safety/backup-2026-08-02-participation-referral-memory-prechange-code` →
`1a762d500976e1c9c04d5454f577c94dcea22a47`; отдельные refs `main`,
`state`, `status`, `ledger` зафиксированы тем же префиксом и прошли read/JSON
restore-smoke до изменения.
...
- Опциональный AI-инспектор больше не блокирует детерминированную диагностику,
  если его provider или конфигурация временно недоступны.
- Ошибка чтения encrypted-конфигурации уведомлений оформляется отдельным
  finding вместо аварийного завершения всего System Health.
- Пересчёт и lifecycle остальных инцидентов продолжаются, поэтому свежие
  здоровые runtime-снимки закрывают устаревшие сбои.

# 2026-07-31 — коррелированный timeout Telegram не карантинит источники по отдельности

- Production-состояние зафиксировало восемь источников
  (`EviLGrannY_WoT`, `IGMMLBB`, `StrayDungeon228`, `m4gastreams`,
  `obshakstaya`, `pandafx`, `shadowkekw`, `zont1x`), которые успешно
  читались за 2–7 минут до почти одновременных трёх `ReadTimeout`. Прежний
  transport признавал коррелированным только полный отказ всех 169 источников,
  поэтому частичный сетевой всплеск ошибочно создал восемь независимых
  критических инцидентов и шестичасовых карантинов.
- Transport теперь повторяет только transient-сбойное подмножество и объединяет
  восстановившиеся результаты с первым проходом. Если после retry одновременно
  остаются не менее трёх transient-ошибок, они фиксируются как общий transport
  outage и не увеличивают индивидуальные quarantine-счётчики.
- Уже записанный кластер transient-карантинов в десятиминутном окне допускается
  к немедленной контрольной перепроверке; изолированный источник сохраняет
  прежний шестичасовой интервал.
- `source_transport_state.status` больше не называется `success`, когда весь
  inventory учтён, но хотя бы один источник завершился ошибкой: такое состояние
  имеет статус `degraded`. System Health показывает `reachable/errors` и может
  закрыть старую primary-ошибку только по более новому полному успешному
  Monitor-проходу. Nightly-ошибка этим доказательством не маскируется.
- Инциденты `source_*` и общий `sources_quarantined` проходят то же окно
  подтверждения, что остальные временные transport/runtime-сигналы.

**Backup перед изменением:**
`backup/2026-07-31-before-transport-diagnostics-repair` →
`d31689965cd7a738c93cc019d98a90813359bd43`.

# 2026-07-30 — реферальное участие учитывает доступность по аккаунтам

- Production-событие `@shadowkekw`, `kekw2` подтвердило, что реферальная
  доступность различается между BetBoom-аккаунтами. Первичный снимок поста
  содержал «Колесико для всех», поэтому обычная карточка была отправлена; позднее
  тот же пост содержал «Колесико для рефов» и recovery корректно сохранил
  `referral_restricted=true`.
- Аккаунт 2 получил подтверждённый `participated`, а аккаунт 1 остался в
  техническом `unconfirmed`. Прежнее правило полностью подавляло итог
  реферального события. Теперь первичная карточка, напоминания и время прокрутки
  по-прежнему бесшумны, пока ни один аккаунт не подтвердил участие. При успехе
  хотя бы одного аккаунта Control Center отправляет один общий итог с состоянием
  каждого аккаунта; `unconfirmed` не называется окончательным отказом.
- Ранее завершённые записи с `notification_policy=referral_suppressed` и
  подтверждённым успехом можно один раз повысить до нового итогового уведомления.
  Идемпотентный completion после этого снова подавляет повторы.
- Диагностическое расхождение рейтинга не относилось к `kekw2`: записи
  `GShikaryan` и `gshikaryan` содержали соответственно 20 и 10 очков, но health
  сравнивал регистронезависимый ожидаемый итог 30 с последним перезаписавшим
  значением 10. Проверка теперь суммирует case-варианты одной source identity.

**Backup перед изменением:**
`backup/before-referral-account-aware-notifications-20260730` →
`24dbafaa583147116d89faa26911b62ce9ad8e41`.
...
- Нули во вкладке «Аналитика», пустой список активных колёс, устаревшее время
  ...
  Это создавало около 5040 запросов в час и самостоятельно исчерпывало installation
  rate limit. Callback теперь всегда отвечает из последнего проверенного локального
  snapshot, а обновление выполняется только фоновым потоком раз в 30 секунд.
  Для одного refresh используются два API-запроса (по одному SHA на ветку), после
  чего семь файлов читаются по immutable raw URL. Это снижает постоянную нагрузку
  на GitHub API с 5040 примерно до 240 запросов в час.
- Runtime state/stats/health загружаются из authoritative ветки `runtime-state`,
  а конфигурационные списки источников — из release-ветки. Ошибка одного файла
  больше не отбрасывает успешно обновлённые компоненты: для сбойного компонента
  сохраняется последняя проверенная версия. Полностью пустой `{}` не может заменить
  непустой state, stats, health или discovery.
- Production-инцидент `CTOM23` восстановлен по первичным данным. Monitor увидел
  событие из `ct0mislove/5495` в 20:12 и запустил workflow, но основной browser
  был пропущен: нажатие пользователем кнопки «✅ Участвую» в Telegram ошибочно
  считалось подтверждением BetBoom. Теперь Telegram-кнопка означает только
  пользовательскую отметку; browser всегда открывает BetBoom и либо подтверждает
  уже принятое вручную участие (`already_participating`), либо нажимает точную
  кнопку участия.
- Legacy-результат `already_marked_in_bot` больше не является success. Активная
  генерация с таким результатом повторно ставится на реальную BetBoom-проверку.
  Результаты содержат `confirmation_method` и `participation_origin`, поэтому
  ручное участие не смешивается с автоматическим.
- Итог `CTOM23` потерялся после успешных account-проходов из-за GitHub API `403
  rate limit`: runtime checkpoint не был опубликован. Финальный workflow теперь
  при таком отказе отправляет owner-scoped резервный итог напрямую через Telegram
  и включает непустую строку по обоим аккаунтам Вячеслава. Аккаунты другого
  владельца в сообщение не попадают; повтор в пределах одного workflow подавляется
  durable marker-файлом runner.
- Исправлен production preflight: self-test второго аккаунта ожидал удалённый
  legacy-ключ `wheel#action:...`, хотя runtime уже использует канонический
  `evt:<generation_id>`. Из-за этого каждый workflow формально был красным ещё до
  browser-прохода.
- Cursor `kolesaBB=71862` теперь проверяется через прямую immutable-страницу даже
  при timeout основного `/s/kolesaBB`. Если точного поста в этом namespace нет,
  cursor безопасно возвращается к последнему доказанному ID и одним recovery-циклом
  догоняет последовательность сообщений. Сетевой сбой самой контрольной проверки
  не считается доказательством неправильного cursor и не вызывает сброс.

**Backup перед изменением:**
`backup/before-ctom23-manual-participation-fix-20260726` →
`010a56b1fad094a3421a8ffc030006b609db1bff`.
...
- «Работа системы» показывает время последнего подтверждённого обхода, номер и
  длительность цикла, а не называет возраст heartbeat временем самой проверки.
- При просроченном heartbeat дополнительно учитывается состояние живого
  `monitor.yml`: запущенный Monitor обозначается отдельно от задержки публикации
  телеметрии.
- Временная ошибка чтения `intelligence_state.json` больше не заменяет последний
  проверенный результат разведки нулём; используется кэш либо снимок exact-SHA.
