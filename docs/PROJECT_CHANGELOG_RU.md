## 2026-09-15 — диагностические прогоны отделены от deployment-ветки

- Устранена первопричина просроченных проверок: успешная разведка 178
  источников теряла результат при конфликте прямого push в быстро меняющийся
  `main`, а transport workflow выполнял полный checkout и допускал отмену
  активного прогона.
- `intelligence_state.json` и `source_transport_state.json` теперь публикуются
  через file-level CAS в `runtime-state`; System Health загружает оба
  авторитетных снимка перед диагностикой.
- Checkout обоих maintenance workflow ограничен одним commit без сохранения
  write-credentials, а уже начатая полная транспортная проверка больше не
  отменяется другим запуском.
- Публичный inventory и добавленные через защищённую конфигурацию приватные
  Telegram-источники учитываются раздельно: transport проверяет полный союз,
  а health сравнивает 178 публичных записей с публичной частью снимка и не
  объявляет исправный приватный источник лишним.

**Backup перед изменением:**
`backup/2026-09-15-runtime-diagnostic-state-fix` →
`7dfe4d61b98545308ab31dece481d9bed6b4b4d7`.

## 2026-09-11 — единое доказательство участия для всех аккаунтов

- Аккаунт 1 переведён с одиночного прямого вызова браузера на тот же
  `participate_with_persistence_proof`, который уже используется Аккаунтом 2
  и `xFLARXx`: клик, авторитетный ответ `/action/join`, затем проверка
  сохранённого участия в новом браузерном контексте.
- Единый путь применяется и в быстрой обработке известного события, и в полном
  recovery-проходе. Добавлен regression-тест, запрещающий возврат Аккаунта 1
  к отдельному сценарию.

**Backup перед изменением:**
`backup/2026-09-11-before-unified-account-proof` →
`b556dc5f69f771a2076577c8106d6f4f95885c77`.

## 2026-09-11 — устранена 404 для headless-браузера автоучастия

- Подтверждена первопричина массовых пропусков автоучастия 10–11 сентября:
  BetBoom возвращал активный `/freestream/[slug]` обычному Chrome, но тот же
  запрос с продуктовым токеном `HeadlessChrome` маршрутизировал на Next.js
  `/404`. Монитор с обычным Chrome User-Agent поэтому подтверждал активное
  событие, а авторизованный Playwright не получал кнопку участия.
- Браузер автоучастия теперь использует фактическую версию запущенного Chrome
  без automation-only токена `HeadlessChrome`. Сессии аккаунтов, URL, клик и
  проверка ответа `/action/join` не изменены.
- Добавлены regression-тесты версии Chrome и безопасного fallback User-Agent.

**Backup перед изменением:**
`backup/2026-09-11-before-headless-user-agent-root-fix` →
`ce470844c7ef7f3506f401638cae15cf53cb4b98`.

## 2026-09-10 — retry активного колеса после временной 404 BetBoom

- Отсутствие кнопки после перезагрузки больше не считается доказательством
  закрытия колеса: без точного текста завершения это retryable
  `button_not_found`. Это покрывает случай, когда API BetBoom ещё показывает
  активное событие, а frontend временно отдаёт страницу 404 или неполный SPA.
- Ошибочно записанные с 2026-09-08 terminal-состояния
  `participation_control_absent_after_reload` автоматически разоружаются и не
  блокируют повторную попытку активного события.
- Кэш проверки профилей теперь привязан к набору аккаунтов, чьи session secrets
  доступны в конкретном workflow step. Переход от primary к account 2 и
  xFLARXx больше не переиспользует отчёт, где эти аккаунты были
  `not_configured`.
- Добавлены regression-тесты для сочетания «активное API-событие + frontend
  404», миграции ложного terminal-состояния и смены account scope кэша.

**Backup перед изменением:**
`backup/2026-09-10-before-active-wheel-404-retry` →
`16ee1584c2ff28296cf30c53d5343eb7c82b9773`.

## 2026-09-06 — диагностика автоучастия и несколько источников одного колеса
...
# 2026-09-04 — ручная подача колеса через Telegram-бота

- В разделе «Управление» и на экране активных колёс добавлена команда
  «Добавить колесо» для владельца и администраторов.
- Прямая ссылка `betboom.ru/freestream/...` передаётся через существующую
  конфликтобезопасную очередь административных действий.
- Monitor проверяет ручную ссылку через тот же BetBoom API и использует
  штатные уведомления, дедупликацию, event/outbox, автоучастие, повторную
  проверку и завершение события.
- Повторная подача одного действующего события не создаёт второе уведомление,
  а технический источник ручной подачи не участвует в рейтинге Telegram-каналов.

**Backup перед изменением:**
`backup/2026-09-04-before-manual-wheel-intake` →
`3f8fc8aab7aed5cd0ab771b5d2b8fddf0ec58499`.

# 2026-09-02 — health соответствует единой модели источников

- Удалены ложные health-инциденты по уже выведенным из production
  `nightly_discovery.py`, `discovery_state.json` и `source_tier_state.json`.
- Health использует актуальный `intelligence_state.json`, а полная транспортная
  проверка единого `public_sources.txt` теперь запускается ежедневно.
- Для каждого устойчиво недоступного источника сохраняется один предметный
  инцидент без дублирующей сводки о карантине.
- Monitor больше не запускает `VK start welcome` на каждой минутной итерации;
  обработчик остаётся владельцем собственного workflow по расписанию.
- Исправлен повреждённый бинарный snapshot этого журнала: восстановлена
  последняя подтверждённая UTF-8 версия из истории репозитория.
...
## 2026-09-06 — дедупликация одного колеса между Telegram-источниками

- Исправлена потеря внутреннего `_bbvg_event_id` при перестройке inline-клавиатуры в `bbvg_monitor_runtime.py`. Теперь разные Telegram-публикации одной точной BetBoom-генерации используют один durable notification identity и не создают повторную первичную карточку.
- Добавлен regression-сценарий реального инцидента `zonertw4`: закрытая публикация `private_2445382077/7815` и публичная `amam0610/72466` с одной канонической генерацией обязаны дать один notification identity.
- Технические алиасы закрытых Telegram-источников вида `private_<peer>` маскируются в пользовательском тексте как «закрытый Telegram-канал»; исходный URL поста и внутренняя source identity для рейтинга/учёта не меняются.
